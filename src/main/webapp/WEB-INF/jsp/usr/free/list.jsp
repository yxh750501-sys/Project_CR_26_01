...
<div class="wrap" style="margin-top:20px;">
  ...
  <div class="hd">
    <h2>자유게시판 <small style="font-size:14px;color:#888;">(정보 공유)</small></h2>
    <c:if test="${not empty loginedUserId}">
      <a href="/usr/free/write" class="btn">+ 글쓰기</a>
    </c:if>
  </div>
    ...
    <tbody>
      <c:choose>
        <c:when test="${empty posts}">
          <tr><td colspan="4" class="empty">등록된 게시글이 없습니다.</td></tr>
        </c:when>
        <c:otherwise>
          <c:forEach var="p" items="${posts}">
            <tr>
              <td>${p.id}</td>
              <td><a class="ttl" href="/usr/free/detail?id=${p.id}"><c:out value="${p.title}"/></a></td>
  ...
  <c:if test="${totalPages > 1}">
    <div class="pager">
      <c:if test="${page > 1}"><a href="/usr/free/list?page=${page-1}">&laquo;</a></c:if>
      <c:forEach begin="1" end="${totalPages}" var="p">
        <c:choose>
          <c:when test="${p == page}"><span class="cur">${p}</span></c:when>
          <c:otherwise><a href="/usr/free/list?page=${p}">${p}</a></c:otherwise>
        </c:choose>
      </c:forEach>
      <c:if test="${page < totalPages}"><a href="/usr/free/list?page=${page+1}">&raquo;</a></c:if>
    </div>
  </c:if>
</div>
</body>

...
public class BeforeActionInterceptor implements HandlerInterceptor {
	...
	public void postHandle(HttpServletRequest req, HttpServletResponse resp, Object handler, ModelAndView mv)
		...
		RunRecommendationService runRecommendationService = runRecommendationServiceProvider.getIfAvailable();
		CenterRecommendationService centerRecommendationService = centerRecommendationServiceProvider.getIfAvailable();

		// 빈이 없으면(스캔 실패 등) 서버가 죽지 않게 그냥 패스
		if (runRecommendationService == null || centerRecommendationService == null) return;

		// 추천 생성 + 조회
		runRecommendationService.ensureGenerated(runId, userId);
		List<Map<String, Object>> runRecommendations =
				runRecommendationService.getRunRecommendationsWithEvidence(runId, userId);
		...
		List<Map<String, Object>> therapyCenterBlocks =
				centerRecommendationService.buildTherapyBlocksFromRunRecommendations(
						runRecommendations,
						sido, sigungu,
						lat, lng, radiusKm,
						2,
